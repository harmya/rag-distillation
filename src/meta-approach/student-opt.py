...
from transformers import GPT2Tokenizer, OPTForCausalLM, DataCollatorForLanguageModeling

device = 0

class DistillDataOPT(Dataset):
...
distill_dataset = DistillDataOPT(data_list, tokenizer)
data_collator = DataCollatorForLanguageModeling(tokenizer=tokenizer, mlm=False)
    
model = OPTForCausalLM.from_pretrained("facebook/opt-125m")

training_args = TrainingArguments(
    output_dir="/scratch/gilbreth/mangla/exps-rag-opt",
